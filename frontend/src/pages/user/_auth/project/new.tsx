...
const questionGroupTitleSeparatorStyles = cva('my-4 bg-gray-400 w-full h-[2px]')
        ...
        if (fileChanges.length > 0) {
          ...
          await Promise.all(
            fileChanges.map(async (change) => {
              if (change.action === 'remove' && change.file.metadata?.id) {
                  ...
                  if (field.key === inputFieldKey) {
                    if (field.type !== 'file') {
                      ...
                      if (uploadedFiles.length > 0) {
                        console.log(
                          '[ProjectPage] Adding uploaded files to draft:',
                          {
                            questionId,
                            inputFieldKey,
      ...
      subSections: group.subSections.map((subsection) => ({
        ...
        questions: subsection.questions.map((question) => ({
          ...
          inputFields: question.inputFields.map((field) => {
            ...
            const sampleValue = getSampleAnswer(question.question, field.type)
              ...
              case 'textinput':
                if (!input.value.value) {
                  ...
                  valid = input.validations.every(
                    (validation) =>
                      validation.safeParse(input.value.value).success,
                  )
                }
                break
              case 'select':
    ...
    <div>
      ...
      {isSaving && (
        <div className="fixed left-0 right-0 top-0 z-10">
          <p className="text-center py-2 bg-gray-200">Saving application...</p>
        </div>
      )}
      <SectionedLayout
