...
function PermissionsPage() {
  ...
  useEffect(() => {
    ...
    fetchUsers();
  }, [accessToken, authLoading, selectedRole, searchQuery, sortOrder, currentPage, push]);

  const handleRoleChange = async (userId: string, newRole: 'admin' | 'investor' | 'regular') => {
  ...
  const handleBulkRoleUpdate = async (newRole: 'admin' | 'investor' | 'regular') => {
              ...
              users.map((user) => (
                <tr key={user.id}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <select
                      value={user.role}
                      onChange={(e) => handleRoleChange(user.id, e.target.value as 'admin' | 'investor' | 'regular')}
          ...
          <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
            <div>
              <p className="text-sm text-gray-700">
                Showing <span className="font-medium">{Math.min((currentPage - 1) * ITEMS_PER_PAGE + 1, totalUsers)}</span>{' '}
                to <span className="font-medium">{Math.min(currentPage * ITEMS_PER_PAGE, totalUsers)}</span> of{' '}
            ...
            <div>
              <nav className="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                ...
                {Array.from({ length: Math.ceil(totalUsers / ITEMS_PER_PAGE) }).map((_, i) => (
                  <button
                    key={i}
