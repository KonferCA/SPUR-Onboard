 ...
 */
export function formatSocialDisplay(
    ...
    try {
        ...
        try {
            ...
            switch (platform) {
                ...
                case SocialPlatform.Discord:
                    return lastPart
                        ? '@' + sanitizeSocialInput(lastPart)
                        : 'Profile Link';
                case SocialPlatform.CustomUrl: {
                    // try to extract domain
                    const domainMatch = urlOrHandle.match(/\/\/([^\/]+)/);
                    return domainMatch
                        ? sanitizeSocialInput(domainMatch[1])
                        : 'Web Link';
                }
                default:
                    return 'Profile Link';
            ...
            // last resort
            console.warn(
                'Failed to parse social link:',
                urlOrHandle,
                fallbackError
            );
            return platform === SocialPlatform.CustomUrl
                ? 'Web Link'
                : 'Profile Link';
        }
    }
 ...
 */
export function processSocialLinksFromApi(
    socialLinks: Array<{ platform: string; url_or_handle: string }> = []
): SocialLink[] {
 ...
 */
export function processSocialLinksForApi(
    socialLinks: SocialLink[] = []
): Array<{ platform: string; url_or_handle: string }> {
    if (!socialLinks || !Array.isArray(socialLinks)) return [];
