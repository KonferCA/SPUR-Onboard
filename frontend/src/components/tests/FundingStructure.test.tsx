...
import { FundingStructure, FundingStructureModel } from '@/components/FundingStructure';
...
describe('FundingStructure', () => {
  ...
  it('renders existing funding structure information when value is provided', () => {
    render(<FundingStructure value={mockTargetStructure} onChange={onChangeMock} />);
    expect(screen.getByText('Target Funding Structure')).toBeInTheDocument();
    expect(screen.getByText(/Target funding: \$100000 CAD for 10% equity/)).toBeInTheDocument();
  });
  ...
  it('toggles investor limit switch', async () => {
    ...
    const switchLabel = screen.getByText('Set a limit on number of investors?');
  ...
  it('closes the modal without saving when cancel is clicked', async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByText('Add Funding Structure')).not.toBeInTheDocument();
    }, { timeout: 1000 });
  });
  ...
  it('displays progress bar with correct percentage', async () => {
    ...
    const progressElements = document.querySelectorAll('.h-full, .bg-gray-200, [style*="width"]');
  ...
  it('handles very large equity percentages gracefully', async () => {
    ...
    if (equityInput) {
      ...
      const filledBar = document.querySelector('.h-full[style*="width: 100%"]');
  ...
  it('initializes with existing value from props', async () => {
    ...
    render(<FundingStructure value={customStructure} onChange={onChangeMock} />);
    
    // Check summary shows correct info
    const summary = screen.getByText(/Target funding: \$250000 CAD for 15% equity/i, { exact: false });
    ...
    const investorSwitch = document.querySelector('[role="switch"][aria-checked="true"]');
