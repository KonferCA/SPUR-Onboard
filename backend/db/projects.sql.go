...
popularity_scores AS (
    SELECT 
        pd.id, pd.company_id, pd.title, pd.description, pd.status, pd.created_at, pd.updated_at, pd.document_count, pd.team_member_count, pd.comment_count,
...
type GetPopularProjectsRow struct {
	...
	DocumentCount     int64         `json:"document_count"`
	TeamMemberCount   int64         `json:"team_member_count"`
	PsPopularityScore float64       `json:"ps_popularity_score"`
}
