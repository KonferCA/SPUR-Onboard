...
func (q *Queries) CountProjectSnapshots(ctx context.Context, projectID string) (int64, error) {
	row := q.db.QueryRow(ctx, countProjectSnapshots, projectID)
	var count int64
	err := row.Scan(&count)
	return count, err
}

const createProjectSnapshot = `-- name: CreateProjectSnapshot :exec
